...
for img in **/*.* **/**/*.*; do
  ...
  case "$base" in
    ...
    *)
      ...
      convert "$img" -pointsize "$size" -gravity southeast -fill "#000000" -draw "text 5,5 'http://raph.es'" -quality 100 "$target"
      # second pass, white text and some compression
      convert "$target" -pointsize "$size" -gravity southeast -fill "#FFFFFF" -draw "text 6,6 'http://raph.es'" -quality 95 "$target"
      # thumbnail
